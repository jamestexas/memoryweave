...
class MemoryIntegrationTest(unittest.TestCase):
    ...
    def _add_test_memories(self):
        ...
        embeddings = [
            (np.array([1.0, 0.0, 0.0, 0.0]), "Personal fact: My name is Alex", {"type": "personal"}),
            (np.array([0.0, 1.0, 0.0, 0.0]), "Factual info: Python is a programming language", {"type": "factual"}),
            (np.array([0.0, 0.0, 1.0, 0.0]), "Opinion: I think AI is fascinating", {"type": "opinion"}),
            (np.array([0.9, 0.1, 0.0, 0.0]), "Personal fact: I live in Seattle", {"type": "personal"}),
            (np.array([0.1, 0.9, 0.0, 0.0]), "Factual info: Seattle is in Washington state", {"type": "factual"}),
        ]
