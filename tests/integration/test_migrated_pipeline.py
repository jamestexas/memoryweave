...
class TestMigratedPipeline:
    ...
    def setup_predictable_test_data(self, embedding_dim: int = 768) -> Dict[str, Any]:
        ...
        personal_query_embedding = personal_query_embedding / np.linalg.norm(personal_query_embedding)
    ...
    def migrated_pipeline(self, test_data):
        ...
        for component in [
            memory_store, vector_store, activation_manager,
            similarity_strategy, hybrid_strategy, two_stage_strategy,
            query_analyzer, query_adapter
        ]:
    ...
    def test_basic_retrieval_functionality(self, legacy_memory, migrated_pipeline, test_data):
        ...
        for i, result in enumerate(migrated_results):
            print(f"  {i+1}. {result.get('content', '')} (score: {result.get('relevance_score', 0):.4f})")
    ...
    def test_query_type_specific_pipelines(self, migrated_pipeline, test_data):
        ...
        for query_type, result_info in results_by_type.items():
            ...
            for i, r in enumerate(result_info["results"][:3]):
                print(f"  {i+1}. {r.get('content', '')} (score: {r.get('relevance_score', 0):.4f})")
    ...
    def test_query_adaptation_parameters(self, migrated_pipeline, test_data):
        ...
        for query_text, query_embedding, expected_type in test_data["queries"]:
            ...
            adaptation_results[expected_type.name] = {
                "query": query_text,
                "parameters": params
            }
        ...
        assert "similarity_threshold" in personal_params, "Missing similarity_threshold for personal query"
        ...
        assert "similarity_threshold" in factual_params, "Missing similarity_threshold for factual query"
