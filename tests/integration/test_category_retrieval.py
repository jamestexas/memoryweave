...
class TestCategoryRetrieval:
    ...
    def test_category_manager_integration(self):
        ...
        for i, embedding in enumerate(embeddings):
            memory.add_memory(
                embedding,
                f"Test memory {i}",
                {"content": f"Memory content {i}", "index": i}
            )
    ...
    def test_category_retrieval_in_pipeline(self):
        ...
        manager.build_pipeline([
            {"component": "category_retrieval", "config": {
                "confidence_threshold": 0.3,
                "max_categories": 3,
                "category_selection_threshold": 0.5,
        ...
        for category, pattern in categories.items():
            ...
            for i in range(3):  # 3 memories per category
                ...
                idx = memory.add_memory(
                    embedding,
                    f"{category} memory {i}",
                    {"category": category, "content": f"{category} content {i}", "index": len(memory.memory_embeddings) - 1}
                )
                ids.append(idx)
        ...
        queries = {
            "cat": {"embedding": categories["cat"] * 0.9 + np.random.randn(128) * 0.1, "text": "Tell me about cats"},
            "dog": {"embedding": categories["dog"] * 0.9 + np.random.randn(128) * 0.1, "text": "Information about dogs"},
            "weather": {"embedding": categories["weather"] * 0.9 + np.random.randn(128) * 0.1, "text": "What's the weather like?"},
            "food": {"embedding": categories["food"] * 0.9 + np.random.randn(128) * 0.1, "text": "What food do you like?"},
        }
        
        # Normalize query embeddings
        for category in queries:
            queries[category]["embedding"] = queries[category]["embedding"] / np.linalg.norm(queries[category]["embedding"])
    ...
    def test_hybrid_category_pipeline(self):
        ...
        try:
            ...
            from memoryweave.components.post_processors import KeywordBoostProcessor, SemanticCoherenceProcessor
        except ImportError:
            # Skip test if required dependencies are not available
        ...
        manager.build_pipeline([
            ...
            {"component": "keyword_boost", "config": {
        ...
        embeddings = [
            (create_test_embedding("cat content", 128), "All about cats", {"type": "animal", "category": "cat"}),
            (create_test_embedding("more cat stuff", 128), "Cat behaviors", {"type": "animal", "category": "cat"}),
            (create_test_embedding("dog content", 128), "Dogs are loyal", {"type": "animal", "category": "dog"}),
            (create_test_embedding("weather today", 128), "Weather forecast", {"type": "weather", "category": "weather"}),
            (create_test_embedding("food recipes", 128), "Cooking recipes", {"type": "food", "category": "food"}),
        ]
