...
class TestBenchmarkConfigurations:
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> BenchmarkMetrics:
        ...
        for query_item in self.test_queries:
            ...
            success, metrics = verify_retrieval_results(
                results, relevant_indices, require_all=False, check_order=False
            )
    ...
    def _config_has_impact(
        ...
        if abs(baseline_metrics.precision - test_metrics.precision) > threshold:
            differences.append(
                f"precision ({baseline_metrics.precision:.4f} vs {test_metrics.precision:.4f})"
            )

        # Check recall difference
        if abs(baseline_metrics.recall - test_metrics.recall) > threshold:
            differences.append(
                f"recall ({baseline_metrics.recall:.4f} vs {test_metrics.recall:.4f})"
            )

        # Check F1 difference
        if abs(baseline_metrics.f1_score - test_metrics.f1_score) > threshold:
            differences.append(
                f"F1 score ({baseline_metrics.f1_score:.4f} vs {test_metrics.f1_score:.4f})"
            )
        ...
        if count_diff >= 0.5:  # At least half a result difference on average
            differences.append(
                f"retrieval count ({baseline_metrics.retrieval_count:.1f} vs {test_metrics.retrieval_count:.1f})"
            )
