...
class TestBenchmarkPerformance:
    ...
    def deterministic_dataset(self):
        ...
        dataset_path = (
            Path(__file__).parent.parent / "test_data" / "deterministic_benchmark_dataset.json"
        )
        ...
        if not dataset_path.exists():
            ...
            test_data = {"memories": memories, "queries": queries}
    ...
    def test_basic_performance_determinism(self, deterministic_dataset, basic_config):
        ...
        first_result = self.run_timed_benchmark(
            dataset_path=deterministic_dataset, config=basic_config, max_queries=5, runs=1
        )
        ...
        time_ratio = second_result["execution_times"][0] / max(
            0.001, first_result["execution_times"][0]
        )
    ...
    def test_query_performance_profile(self, deterministic_dataset, advanced_config):
        ...
        result = self.run_timed_benchmark(
            dataset_path=deterministic_dataset, config=advanced_config, max_queries=5, runs=1
        )
