...
class TestDynamicContextAdapter:
    ...
    def test_memory_size_adaptation(self):
        ...
        large_result = self.adapter.process_query("What is the capital of France?", large_memory_context)
        small_result = self.adapter.process_query("What is the capital of France?", small_memory_context)
        ...
        assert large_params.get("use_progressive_filtering", False), "Progressive filtering should be enabled for large stores"
        assert not small_params.get("use_progressive_filtering", False), "Progressive filtering should be disabled for small stores"
        
        # Large stores should have higher similarity weight to compensate for reduced associative/activation
        assert large_params["similarity_weight"] > small_params["similarity_weight"], "Large stores should emphasize similarity more"
        
        # Large stores should have reduced activation influence
        assert large_params["activation_weight"] < small_params["activation_weight"], "Large stores should reduce activation weight"
        ...
        assert large_combined < small_combined, "Large stores should have lower combined activation and associative weights"
    ...
    def test_query_type_adaptation(self):
        ...
        assert personal_params["confidence_threshold"] > factual_params["confidence_threshold"], "Personal queries should have higher confidence threshold"
        
        # Temporal queries should emphasize temporal context more
        assert temporal_params["temporal_weight"] > personal_params["temporal_weight"], "Temporal queries should emphasize temporal context"
        assert temporal_params["temporal_weight"] > factual_params["temporal_weight"], "Temporal queries should emphasize temporal context"
        
        # Factual queries should emphasize similarity more
        assert factual_params["similarity_weight"] > personal_params["similarity_weight"] or \
               factual_params["similarity_weight"] > temporal_params["similarity_weight"], "Factual queries should emphasize similarity"
    ...
    def test_query_characteristics_adaptation(self):
        ...
        specific_result = self.adapter.process_query("Why is Python considered a readable programming language?", specific_context)
        ...
        assert specific_params["similarity_weight"] > vague_params["similarity_weight"], "Specific queries should emphasize similarity"
        
        # Vague queries should have higher associative weight to find related info
        assert vague_params["associative_weight"] > specific_params["associative_weight"], "Vague queries should emphasize associative links"
        
        # Vague queries typically have higher confidence threshold to filter noise
        assert vague_params["confidence_threshold"] >= specific_params["confidence_threshold"], "Vague queries should have higher confidence threshold"
    ...
    def test_adaptation_strength_controls_change(self):
        ...
        if strong_params["confidence_threshold"] > default_confidence:
            assert strong_params["confidence_threshold"] > weak_params["confidence_threshold"], "Strong adapter should increase confidence more"
            
        # For parameters that should decrease
        if strong_params["associative_weight"] < 0.3:
            assert strong_params["associative_weight"] < weak_params["associative_weight"], "Strong adapter should decrease associative weight more"
        
        # For boolean parameters (if they exist in result)
        if "use_progressive_filtering" in strong_params and "use_progressive_filtering" in weak_params:
    ...
    def test_adaptation_history_retention(self):
        ...
        assert len(self.adapter.adaptation_history) == 3, "History should be limited to max_history_size"
        
        # Most recent query should be last in history
        assert self.adapter.adaptation_history[-1]["query"] == queries[-1], "Most recent query should be last in history"
        
        # Earliest query should be dropped
        for history_item in self.adapter.adaptation_history:
            assert history_item["query"] != queries[0], "Earliest query should be dropped from history"
