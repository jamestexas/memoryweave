...
def test_retrieval_configurations(data, embedding_model):
    ...
    print(f"Testing with {len(personal_queries)} personal queries and {len(factual_queries)} factual queries")
    ...
    for config_name, config in configurations.items():
        ...
        overall_precision = (personal_results["precision"] * len(personal_queries) + 
                            factual_results["precision"] * len(factual_queries)) / len(data)
        overall_recall = (personal_results["recall"] * len(personal_queries) + 
                         factual_results["recall"] * len(factual_queries)) / len(data)
        ...
        print(f"Personal queries: Precision={personal_results['precision']:.3f}, Recall={personal_results['recall']:.3f}, F1={personal_results['f1']:.3f}")
        print(f"Factual queries: Precision={factual_results['precision']:.3f}, Recall={factual_results['recall']:.3f}, F1={factual_results['f1']:.3f}")
