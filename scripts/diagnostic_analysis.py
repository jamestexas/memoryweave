...
class DiagnosticAnalysis:
    ...
    def __init__(self, model_name="sentence-transformers/all-MiniLM-L6-v2"):
        ...
        self.retriever = ContextualRetriever(
            memory=self.memory, embedding_model=self.embedding_model
        )
    ...
    def analyze_query_classification(self):
        ...
        with open("diagnostic_output/query_classification.json", "w") as f:
            json.dump({"accuracy": accuracy, "misclassified": misclassified}, f, indent=2)
    ...
    def analyze_embedding_space(self):
        ...
        all_queries = self.personal_queries + self.factual_queries
        query_embeddings = np.array([self.embedding_model.encode(q["query"]) for q in all_queries])
        ...
        tsne = TSNE(
            n_components=2, random_state=42, perplexity=min(30, len(combined_embeddings) - 1)
        )
        ...
        avg_personal_similarity = (
            np.mean([d["similarity"] for d in personal_distances]) if personal_distances else 0
        )
        avg_factual_similarity = (
            np.mean([d["similarity"] for d in factual_distances]) if factual_distances else 0
        )
    ...
    def analyze_threshold_optimization(self):
            ...
            results.append(
                {
                    ...
                    "overall": {
                        "precision": (
                            personal_metrics["precision"] * len(self.personal_queries)
                            + factual_metrics["precision"] * len(self.factual_queries)
                        )
                        / len(all_queries),
                        "recall": (
                            personal_metrics["recall"] * len(self.personal_queries)
                            + factual_metrics["recall"] * len(self.factual_queries)
                        )
                        / len(all_queries),
                        "f1": (
                            personal_metrics["f1"] * len(self.personal_queries)
                            + factual_metrics["f1"] * len(self.factual_queries)
                        )
                        / len(all_queries),
                    },
                }
        ...
        print(
            f"Personal queries: {optimal_thresholds['personal']['threshold']:.2f} (F1: {optimal_thresholds['personal']['f1']:.3f})"
        )
        print(
            f"Factual queries: {optimal_thresholds['factual']['threshold']:.2f} (F1: {optimal_thresholds['factual']['f1']:.3f})"
        )
        print(
            f"Overall: {optimal_thresholds['overall']['threshold']:.2f} (F1: {optimal_thresholds['overall']['f1']:.3f})"
        )

        # Save results
        with open("diagnostic_output/threshold_optimization.json", "w") as f:
            json.dump({"results": results, "optimal_thresholds": optimal_thresholds}, f, indent=2)
    ...
    def analyze_failed_queries(self):
        ...
        if failed_queries:
            ...
            for i, item in enumerate(failed_queries[:3]):  # Show first 3
                print(
                    f'{i + 1}. Query: "{item["query"]}" ({item["category"]}, detected as {item["detected_type"]})'
                )
                ...
                for j, mem in enumerate(item["expected_memories"]):
                    print(
                        f"     {j + 1}. {mem['text']} (sim: {mem['similarity']:.3f}, rank: {mem['rank']})"
                    )
                print()
    ...
    def run_all_analyses(self):
        ...
        summary = {
            ...
            "embedding_analysis": {
                "avg_personal_similarity": np.mean(
                    [
                        d["similarity"]
                        for d in embedding_analysis_results
                        if d["query_type"] == "personal"
                    ]
                )
                if embedding_analysis_results
                else 0,
                "avg_factual_similarity": np.mean(
                    [
                        d["similarity"]
                        for d in embedding_analysis_results
                        if d["query_type"] != "personal"
            ...
            "failed_queries": {
                ...
                "personal_count": sum(
                    1 for q in failed_query_analysis if q["detected_type"] == "personal"
                ),
                "factual_count": sum(
                    1 for q in failed_query_analysis if q["detected_type"] != "personal"
                ),
        ...
        print(
            f"Average similarity for personal queries: {summary['embedding_analysis']['avg_personal_similarity']:.3f}"
        )
        print(
            f"Average similarity for factual queries: {summary['embedding_analysis']['avg_factual_similarity']:.3f}"
        )
        print(
            f"Optimal threshold for personal queries: {summary['threshold_optimization']['personal']['threshold']:.2f} (F1: {summary['threshold_optimization']['personal']['f1']:.3f})"
        )
        print(
            f"Optimal threshold for factual queries: {summary['threshold_optimization']['factual']['threshold']:.2f} (F1: {summary['threshold_optimization']['factual']['f1']:.3f})"
        )
        print(
            f"Failed queries: {summary['failed_queries']['count']} total ({summary['failed_queries']['personal_count']} personal, {summary['failed_queries']['factual_count']} factual)"
        )
