...
from memoryweave.utils.analysis import (
    analyze_query_similarities,
    analyze_retrieval_performance,
    visualize_memory_categories,
)
    ...
    for category, queries in queries_by_category.items():
        ...
        for query, expected, _ in queries[:5]:  # Test first 5 queries of each category
            ...
            for item in retrieved:
                if (
                    expected.lower() in str(item.get("text", "")).lower()
                    or expected.lower() in str(item.get("description", "")).lower()
                ):
                    found = True
                    break
...
def test_enhanced_retrieval(memory_system, queries_by_category):
    ...
    for category, queries in queries_by_category.items():
        ...
        test_queries = []
        for i, (query, expected, _) in enumerate(
            queries[:5]
        ):  # Test first 5 queries of each category
            # Find memory indices that contain the expected answer
            expected_indices = []
            for j, metadata in enumerate(memory_system["memory"].memory_metadata):
                if (
                    expected.lower() in str(metadata.get("text", "")).lower()
                    or expected.lower() in str(metadata.get("description", "")).lower()
                ):
...
def main():
    ...
    category_stats = visualize_memory_categories(
        memory_system=memory_system, save_path="output/memory_categories.png"
    )
    ...
    for category, results in query_analysis.items():
        ...
        print(
            f"{category.capitalize()} queries: {found_count}/{len(results)} successful retrievals"
        )
