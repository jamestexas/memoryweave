...
class SimilarityDistributionAnalyzer:
    ...
    def _create_synthetic_data(self):
        ...
        # Factual queries
        factual_queries = [
            {"query": "What is the capital of France?", "expected": "The capital of France is Paris."},
            {"query": "Who wrote Hamlet?", "expected": "William Shakespeare wrote Hamlet."},
            {"query": "What is the tallest mountain?", "expected": "Mount Everest is the tallest mountain on Earth."},
            {"query": "What is the speed of light?", "expected": "The speed of light is approximately 299,792,458 meters per second."},
            {"query": "When was the Declaration of Independence signed?", "expected": "The Declaration of Independence was signed on July 4, 1776."}
        ]
        ...
        print(f"Created {len(self.personal_queries)} personal queries and {len(self.factual_queries)} factual queries")
        print(f"Added {len(self.all_memories)} memories to the system")
    ...
    def analyze_similarity_distributions(self):
        ...
        # Process each query
        for query_type, queries in [("personal", self.personal_queries), ("factual", self.factual_queries)]:
            for query in queries:
                ...
                irrelevant_indices = [i for i in range(len(similarities)) if i not in relevant_indices]
                irrelevant_similarities = similarities[irrelevant_indices] if irrelevant_indices else []
        ...
        for query_type in ["personal", "factual"]:
            ...
            print(f"  Relevant similarities: n={stats[query_type]['relevant']['count']}, mean={stats[query_type]['relevant']['mean']:.3f}, median={stats[query_type]['relevant']['median']:.3f}")
            print(f"  Irrelevant similarities: n={stats[query_type]['irrelevant']['count']}, mean={stats[query_type]['irrelevant']['mean']:.3f}, median={stats[query_type]['irrelevant']['median']:.3f}")
        ...
        if similarity_data["personal"]["relevant"]:
            plt.hist(similarity_data["personal"]["relevant"], bins=20, alpha=0.7, label="Relevant", color="green")
        if similarity_data["personal"]["irrelevant"]:
            plt.hist(similarity_data["personal"]["irrelevant"], bins=20, alpha=0.7, label="Irrelevant", color="red")
        ...
        if similarity_data["factual"]["relevant"]:
            plt.hist(similarity_data["factual"]["relevant"], bins=20, alpha=0.7, label="Relevant", color="green")
        if similarity_data["factual"]["irrelevant"]:
            plt.hist(similarity_data["factual"]["irrelevant"], bins=20, alpha=0.7, label="Irrelevant", color="red")
    ...
    def analyze_optimal_thresholds(self, similarity_data):
        ...
        for query_type, color in [("personal", "blue"), ("factual", "red")]:
            ...
            plt.plot(recall_values, precision_values, color=color, label=f"{query_type.capitalize()} Queries")
            
            # Mark optimal threshold
            optimal_idx = np.abs(thresholds - results[query_type]["threshold"]).argmin()
            plt.scatter(recall_values[optimal_idx], precision_values[optimal_idx], color=color, s=100, marker='x')
            plt.annotate(
    ...
    def analyze_threshold_impact(self, similarity_data):
        ...
        for i, metric_name in enumerate(["precision", "recall", "f1"]):
            ...
            for query_type, color, marker in [("personal", "blue", "o"), ("factual", "red", "s")]:
                ...
                plt.plot(thresholds, metric_values, color=color, marker=marker, label=f"{query_type.capitalize()} Queries")
