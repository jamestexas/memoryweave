        ...
        if os.path.exists("diagnostic_output/failed_queries.json"):
            with open("diagnostic_output/failed_queries.json") as f:
                ...
                findings["failed_queries"] = {
                    ...
                    "personal_count": sum(
                        1 for q in failed_queries if q["detected_type"] == "personal"
                    ),
                    "factual_count": sum(
                        1 for q in failed_queries if q["detected_type"] != "personal"
                    ),
...
def generate_report(execution_results, findings):
    ...
    if findings:
        ...
        if (
            "similarity_distributions" in findings
            and "optimal_thresholds" in findings["similarity_distributions"]
        ):
            personal_threshold = findings["similarity_distributions"]["optimal_thresholds"][
                "personal"
            ]["threshold"]
            factual_threshold = findings["similarity_distributions"]["optimal_thresholds"][
                "factual"
            ]["threshold"]
        ...
        if "embedding_quality" in findings:
            if "intra_set_similarity" in findings["embedding_quality"]:
                related_similarity = findings["embedding_quality"]["intra_set_similarity"][
                    "avg_similarity_related"
                ]
                unrelated_similarity = findings["embedding_quality"]["intra_set_similarity"][
                    "avg_similarity_unrelated"
                ]
        ...
        if "failed_queries" in findings:
            if findings["failed_queries"]["count"] > 0:
                personal_ratio = (
                    findings["failed_queries"]["personal_count"]
                    / findings["failed_queries"]["count"]
                )
    ...
    if "embedding_quality" in findings:
        ...
        """
        if "intra_set_similarity" in findings["embedding_quality"]:
            related = findings["embedding_quality"]["intra_set_similarity"][
                "avg_similarity_related"
            ]
            unrelated = findings["embedding_quality"]["intra_set_similarity"][
                "avg_similarity_unrelated"
            ]
            html_report += f"""
            <div class="finding">
...
def main():
    ...
    analyses = [
        ...
        {
            "script": "analyze_similarity_distributions.py",
            "description": "Similarity Distribution Analysis",
        },
    ]
