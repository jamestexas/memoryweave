...
try:
    ...
    logger.warning("[bold yellow]WARNING: Mock embeddings will not represent semantic similarity accurately[/bold yellow]")
    logger.warning("[yellow]Consider installing sentence-transformers package for accurate evaluation[/yellow]")
...
class RealWorldEvaluator:
        ...
        if config.retriever_type == "baseline":
            ...
            try:
                ...
                if hasattr(retriever, "memory_manager"):
                    if hasattr(retriever.memory_manager, "disable_all_pipeline_components"):
                        ...
                        logger.info("[green]Disabled all pipeline components for baseline config[/green]")
                    elif hasattr(retriever.memory_manager, "pipeline_components"):
                        retriever.memory_manager.pipeline_components = {}
                        logger.info("[green]Cleared pipeline components for baseline config[/green]")
            ...
            if hasattr(retriever, "memory_manager"):
                try:
                    ...
                    pipeline_components = getattr(retriever.memory_manager, "pipeline_components", {})
                    if "memory_decay" in pipeline_components:
                        logger.info("[green]Disabling memory decay component for contextual config[/green]")
                ...
                try:
                    ...
                    if "memory_decay" in pipeline_components:
                        if not hasattr(pipeline_components["memory_decay"], "process_query"):
                            logger.info("[green]Working around memory decay component issue[/green]")
                            ...
                            if not config.memory_decay_enabled:
                                ...
                                pipeline_components["memory_decay"].process_query = MethodType(temp_process_query, pipeline_components["memory_decay"])
                except Exception as e:
            ...
            logger.warning(f"[yellow]Unknown retriever type '{config.retriever_type}', using default[/yellow]")
            retriever = Retriever(memory=memory, embedding_model=embedding_model)
            retriever.initialize_components()
        
        return memory, retriever
    
    def run_evaluation(self, save_path: Optional[str] = None, debug: bool = False) -> List[EvaluationResults]:
        ...
        with Progress() as progress:
            ...
            for config in self.configs:
                ...
                try:
                    ...
                    for query_data in self.test_queries:
                        ...
                        if not expected_ids and not query_data.get("category") == "out-of-domain":
                            logger.debug(f"[yellow]No expected IDs found for query: {query}[/yellow]")
                        ...
                        try:
                            ...
                            logger.warning(f"[yellow]Error processing query '{query}': {e}[/yellow]")
                    ...
                    logger.error(f"[bold red]Error evaluating config '{config.name}': {e}[/bold red]")
                    # Skip to next config
                ...
                if expected_indices and retrieved_indices:
                    ...
                    for idx, (query_data, expected, retrieved) in enumerate(zip(self.test_queries, expected_indices, retrieved_indices)):
                        query = query_data["query"]
                        ...
                        if USE_REAL_EMBEDDINGS and retrieved and query:
                            ...
                            for memory_id in retrieved:
                                for memory in self.memories:
                                    if memory.metadata.get("memory_id") == memory_id and memory.embedding is not None:
                    ...
                    logger.warning(f"[yellow]No results data for {config.name}, adding empty result[/yellow]")
                    result = EvaluationResults(
                        config=config,
                        setup_time=setup_time,
                        query_times=[0.0],
    ...
    def _find_memories_with_text(self, text: str, query: str = "") -> Set[int]:
        ...
        if len(matching_ids) > 10:
            logger.debug(f"[yellow]Warning: Found {len(matching_ids)} matching memories for '{text[:30]}...' - this may indicate an issue with matching logic[/yellow]")
    ...
    def _report_results(self) -> None:
        ...
        console.print(f"[bold green]Best performing: {best_result.config.name} (Overall: {best_result.overall_score:.4f})[/bold green]")
        
        # Print fastest configuration
        fastest = min(self.results, key=lambda r: r.avg_query_time)
        console.print(f"[bold green]Fastest: {fastest.config.name} (Avg time: {fastest.avg_query_time:.4f}s)[/bold green]")
        
        # Print best precision and recall
        best_precision = max(self.results, key=lambda r: r.precision)
        console.print(f"[green]Best precision: {best_precision.config.name} ({best_precision.precision:.4f})[/green]")
        
        best_recall = max(self.results, key=lambda r: r.recall)
        console.print(f"[green]Best recall: {best_recall.config.name} ({best_recall.recall:.4f})[/green]")
        ...
        if USE_REAL_EMBEDDINGS:
            ...
            console.print(f"[green]Best semantic similarity: {best_semantic.config.name} ({best_semantic.semantic_similarity:.4f})[/green]")
...
def main():
    ...
    if args.single_config:
        ...
        if args.single_config == "baseline":
            ...
            configs = [c for c in configs if c.retriever_type == "component" and "Optimized" not in c.name]
