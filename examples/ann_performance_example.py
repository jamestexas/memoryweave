...
def benchmark_vector_stores(memory_counts: List[int], dimension: int = 768, 
                           num_queries: int = 10, k: int = 10,) -> Dict[str, Dict[str, List[float]]]:
...
def plot_results(results: Dict[str, Dict[str, List[float]]], output_path: str = "vector_store_benchmark.png"):
    ...
    for i, (store_name, store_data) in enumerate(results.items()):
        ax1.plot(memory_sizes, store_data["search_time"], marker="o", label=store_name, color=colors[i])
        ax2.plot(memory_sizes, store_data["add_time"], marker="o", label=store_name, color=colors[i])
