...
def test_memory_with_vigilance_strategy(strategy, tech_memories, travel_memories, food_memories, embedding_model):
    ...
    for concept, concept_memories in memories_list:
        ...
        for text in concept_memories:
            ...
            if memory_index % 5 == 0 or memory_index == len(tech_memories) + len(travel_memories) + len(food_memories):
                console.print(f"  Memory #{memory_index}: Vigilance = {memory.vigilance_threshold:.3f}, Categories = {stats['num_categories']}", 
                          style="vigilance")
    
    # Print final category statistics
    console.print("\n[bold]Final Category Statistics:[/bold]", style="category")
    stats = memory.get_category_statistics()
    console.print(f"Number of categories formed: [bold]{stats['num_categories']}[/bold] (with vigilance = {memory.vigilance_threshold:.3f})", 
                 style="category")
...
def main():
    console.print(Panel.fit("Testing Dynamic Vigilance Strategies for ART-Inspired Memory Clustering", style="title"))
    ...
    for strategy, stats in results.items():
        table.add_row(
            strategy.title(),
            str(stats["categories"]),
            f"{stats['final_vigilance']:.3f}"
        )
