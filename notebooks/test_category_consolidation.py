...
def generate_test_data():
    ...
    programming_memories = [
        # Python (4)
        {"text": "Python is a high-level programming language known for readability and simplicity.", "subtopic": "python"},
        {"text": "Python's syntax allows programmers to express concepts in fewer lines of code.", "subtopic": "python"},
        {"text": "Python supports multiple programming paradigms, including procedural and object-oriented.", "subtopic": "python"},
        {"text": "Python has a large standard library and active community support.", "subtopic": "python"},
        
        # JavaScript (4)
        {"text": "JavaScript is the primary language for web development and browser interactions.", "subtopic": "javascript"},
        {"text": "JavaScript enables interactive web pages and is essential to web applications.", "subtopic": "javascript"},
        {"text": "JavaScript is a prototype-based language with first-class functions.", "subtopic": "javascript"},
        {"text": "JavaScript frameworks like React and Angular help build complex web applications.", "subtopic": "javascript"},
        
        # Rust (4)
        {"text": "Rust provides memory safety without garbage collection.", "subtopic": "rust"},
        {"text": "Rust's ownership system prevents memory bugs at compile time.", "subtopic": "rust"},
        {"text": "Rust combines high-level ergonomics with low-level control.", "subtopic": "rust"},
        {"text": "Rust is increasingly used for systems programming and performance-critical applications.", "subtopic": "rust"}
    ]
    
    # Machine Learning memories (12)
    ml_memories = [
        # Neural Networks (4)
        {"text": "Neural networks are composed of layers of interconnected nodes.", "subtopic": "neural_networks"},
        {"text": "Deep neural networks have transformed computer vision and natural language processing.", "subtopic": "neural_networks"},
        {"text": "Convolutional neural networks are specialized for processing grid-like data such as images.", "subtopic": "neural_networks"},
        {"text": "Recurrent neural networks are designed to recognize patterns in sequences of data.", "subtopic": "neural_networks"},
        
        # Transformers (4)
        {"text": "Transformer models use self-attention mechanisms to process sequential data.", "subtopic": "transformers"},
        {"text": "GPT models are based on the transformer architecture for natural language processing.", "subtopic": "transformers"},
        {"text": "BERT is a transformer-based model designed for understanding context in text.", "subtopic": "transformers"},
        {"text": "Transformer models have achieved state-of-the-art results in many NLP tasks.", "subtopic": "transformers"},
        
        # Reinforcement Learning (4)
        {"text": "Reinforcement learning involves agents learning through trial and error.", "subtopic": "reinforcement_learning"},
        {"text": "In reinforcement learning, agents aim to maximize cumulative rewards.", "subtopic": "reinforcement_learning"},
        {"text": "Q-learning is a value-based reinforcement learning algorithm.", "subtopic": "reinforcement_learning"},
        {"text": "Policy gradient methods directly optimize the policy in reinforcement learning.", "subtopic": "reinforcement_learning"}
    ]
    
    # Cuisine memories (12)
    cuisine_memories = [
        # Italian (4)
        {"text": "Italian cuisine is known for pasta, pizza, and a variety of regional dishes.", "subtopic": "italian"},
        {"text": "Italian cooking emphasizes high-quality, fresh ingredients.", "subtopic": "italian"},
        {"text": "Italian cuisine varies greatly by region, from seafood in coastal areas to meat inland.", "subtopic": "italian"},
        {"text": "Traditional Italian meals often include multiple courses, from antipasto to dolce.", "subtopic": "italian"},
        
        # Japanese (4)
        {"text": "Japanese cuisine includes sushi, sashimi, and a variety of noodle dishes.", "subtopic": "japanese"},
        {"text": "Japanese cooking emphasizes seasonal ingredients and beautiful presentation.", "subtopic": "japanese"},
        {"text": "Umami, the fifth taste, is an important concept in Japanese cuisine.", "subtopic": "japanese"},
        {"text": "Traditional Japanese meals follow the ichiju-sansai structure: one soup, three sides.", "subtopic": "japanese"},
        
        # Mexican (4)
        {"text": "Mexican cuisine features corn, beans, and chili peppers as staple ingredients.", "subtopic": "mexican"},
        {"text": "Mexican food is known for its vibrant flavors and diverse regional styles.", "subtopic": "mexican"},
        {"text": "Traditional Mexican cooking techniques include nixtamalization of corn.", "subtopic": "mexican"},
        {"text": "Mexican cuisine has influenced cooking styles worldwide, especially in the Americas.", "subtopic": "mexican"}
    ]
    
    # Cross-domain connections (6)
    cross_domain_memories = [
        # Python + ML
        {"text": "Python is the most popular language for machine learning and data science.", "subtopic": "python_ml"},
        {"text": "Libraries like TensorFlow and PyTorch implement neural networks in Python.", "subtopic": "python_ml"},
        
        # JavaScript + ML
        {"text": "TensorFlow.js allows running machine learning models in JavaScript.", "subtopic": "js_ml"},
        {"text": "JavaScript frameworks are increasingly incorporating ML capabilities for web applications.", "subtopic": "js_ml"},
        
        # Cooking + Technology
        {"text": "Machine learning is being used to analyze flavors and create new recipes.", "subtopic": "tech_cooking"},
        {"text": "Computational gastronomy uses data science to understand food pairings and cooking techniques.", "subtopic": "tech_cooking"}
    ]
...
def visualize_category_graph(memory, title="Category Similarity Graph"):
    ...
    try:
        ...
        console.print(f"[success]Category graph visualization saved as '{title.lower().replace(' ', '_')}.png'[/success]")
...
def test_category_consolidation():
    ...
    console.print("\n[bold]Initializing memory with high vigilance but with consolidation enabled[/bold]", style="section")
    ...
    console.print("\n[bold]Adding memories to both systems and tracking category formation...[/bold]", style="section")
    ...
    for i, (text, category, subtopic) in enumerate(all_memories):
        ...
        memory_mappings_consolidation.append((idx_consolidation, cat_idx_consolidation, category, subtopic))
        ...
        if (i + 1) % 10 == 0 or i == len(all_memories) - 1:
            console.print(f"Added {i+1} memories - Consolidation: {consolidation_categories[-1]} categories, " +
            ...
            if i > 0 and consolidation_categories[-1] < consolidation_categories[-2]:
                console.print(f"  [success]Consolidation occurred![/success] Reduced from {consolidation_categories[-2]} to {consolidation_categories[-1]} categories")
    ...
    comparison_table.add_row(
        ...
        f"{avg_size_consolidation:.2f}",
        f"{avg_size_baseline:.2f}"
    )
    ...
    comparison_table.add_row(
        "Category Size StdDev", 
        f"{std_size_consolidation:.2f}",
        f"{std_size_baseline:.2f}"
    )
    ...
    comparison_table.add_row(
        ...
        str(max_size_consolidation),
        str(max_size_baseline)
    )
    ...
    for cat_idx, count in sorted(consolidation_stats["memories_per_category"].items(), 
    ...
    for threshold in thresholds:
        ...
        console.print(f"Threshold {threshold:.1f}: {initial_count} → {final_count} categories (avg size: {avg_size:.2f})")
    ...
    console.print("[success]Threshold comparison chart saved as 'consolidation_thresholds.png'[/success]")
    ...
    for query in test_queries:
        ...
        if results_consolidation:
            for i, (idx, score, metadata) in enumerate(results_consolidation):
                ...
                console.print(f"  {i+1}. [{category}:{subtopic}] {metadata.get('text', '')[:50]}... (Score: {score:.3f})")
        else:
