...
def main():
    ...
    for text in tech_memories:
        embedding, metadata = encoder.encode_concept(concept="technology", description=text)
    ...
    for text in travel_memories:
        embedding, metadata = encoder.encode_concept(concept="travel", description=text)
        idx = memory.add_memory(embedding, text, metadata)
    ...
    for text in food_memories:
        embedding, metadata = encoder.encode_concept(concept="food", description=text)
        idx = memory.add_memory(embedding, text, metadata)
    ...
    console.print(
        f"Number of categories formed: [bold]{stats['num_categories']}[/bold]", style="category"
    )
    ...
    embedding, metadata = encoder.encode_concept(concept="technology", description=new_tech_memory)
    ...
    for query in test_queries:
        ...
        standard_results = memory.retrieve_memories(query_embedding, top_k=3, use_categories=False)

        if standard_results:
            for idx, score, metadata in standard_results:
                original_idx, text, category = [(i, t, c) for i, t, c in all_memories if i == idx][
                    0
                ]
