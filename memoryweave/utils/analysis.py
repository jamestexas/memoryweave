...
def analyze_query_similarities(
    ...
    results = {
        ...
        "retrieved_scores": [item.get("relevance_score") for item in retrieved 
                            if isinstance(item.get("memory_id"), int)],
    ...
    if expected_relevant_indices:
        ...
        if retrieved_indices:
            precision = sum(1 for idx in retrieved_indices if idx in expected_relevant_indices) / len(retrieved_indices)
        else:
            precision = 0.0
            
        if expected_relevant_indices:
            recall = sum(1 for idx in retrieved_indices if idx in expected_relevant_indices) / len(expected_relevant_indices)
        else:
            recall = 1.0
    ...
    if plot:
        ...
        plt.axvline(x=retriever.confidence_threshold, color='r', linestyle='--', 
        ...
        # Mark retrieved memories
        if retrieved_indices:
            plt.bar(retrieved_indices, memory.activation_levels[retrieved_indices], 
                   color='green', alpha=0.7, label='Retrieved')
        
        # Mark expected memories if provided
        if expected_relevant_indices:
            plt.bar(expected_relevant_indices, memory.activation_levels[expected_relevant_indices], 
                   color='blue', alpha=0.7, label='Expected')
    ...
    for params in parameter_variations:
        ...
        for query, expected_indices in test_queries:
            ...
            metrics.append({
                ...
                "correct_retrieved": [idx for idx in retrieved_indices if idx in expected_indices],
