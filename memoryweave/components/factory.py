...
def create_memory_system(config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    core_category_manager = CoreCategoryManager(
        embedding_dim=memory_config.get("embedding_dim", 768),
        **category_config
    )
    ...
    category_adapter = CategoryAdapter(
        core_category_manager=core_category_manager,
        component_category_manager=category_manager
    )
    ...
    try:
        ...
        manager.register_component("two_stage_retrieval", category_retrieval)  # Use category retrieval as two-stage for tests
    except ImportError:
