...
class DynamicThresholdConfig(BaseModel):
    ...
    enable_query_type_adaptation: bool = Field(True, description="Enable query type specific threshold adjustments")
    enable_distribution_based_adjustment: bool = Field(True, description="Enable adjustment based on score distribution")
    enable_feedback_learning: bool = Field(False, description="Enable learning from feedback (if available)")
...
class DynamicThresholdAdjuster(PostProcessor):
    ...
    def _update_thresholds(self, context: Dict[str, Any]) -> None:
        ...
        elif avg_score > self.target_min_relevance * 1.5 and avg_result_count < self.target_result_count:
        ...
        if self.enable_distribution_based_adjustment:
            ...
            if gap_ratio > 2.0:  # Large gap between relevant and irrelevant results
                ...
                threshold_adjustment -= self.adjustment_step * 0.3  # Lower threshold to get more diverse results
        ...
        self.current_threshold = (1 - self.smoothing_factor) * self.current_threshold + self.smoothing_factor * new_threshold
        ...
        if self.enable_query_type_adaptation and query_type in self.query_type_thresholds:
            ...
            if type_metrics:
                ...
                type_avg_score = np.mean([m["avg_score"] for m in type_metrics if m["avg_score"] > 0])
                ...
                elif type_avg_count > self.target_result_count * 1.5 and type_avg_score < self.target_min_relevance:
