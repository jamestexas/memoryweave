...
class DynamicContextAdapter(Component):
    ...
    def _extract_signals(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        signals["query_specificity"] = min(
            1.0, (keyword_ratio * 0.7) + (0.3 if signals["has_entity"] else 0)
        )
        ...
        if activation_manager and hasattr(activation_manager, "get_activated_memories"):
            ...
            signals["activation_density"] = (
                len(activations) / max(1, signals["memory_size"])
                if signals["memory_size"] > 0
                else 0
            )
            signals["max_activation"] = (
                max([v for k, v in activations.items()]) if activations else 0
            )
        else:
    ...
    def _adapt_for_memory_size(
        ...
        if memory_size > 500:  # Large memory stores
            ...
            adapted["similarity_weight"] = min(
                0.8, params["similarity_weight"] + weight_reduction * 0.7
            )

            # Set temporal weight so weights sum to 1.0
            adapted["temporal_weight"] = 1.0 - (
                adapted["similarity_weight"]
    ...
    def _adapt_for_query_type(
        ...
        if query_type == "personal":
            ...
            adapted["confidence_threshold"] = min(
                0.9, params["confidence_threshold"] + 0.15 * adaptation_amount
            )
            adapted["similarity_weight"] = min(
                0.8, params["similarity_weight"] + 0.1 * adaptation_amount
            )
            adapted["activation_weight"] = min(
                0.3, params["activation_weight"] + 0.1 * adaptation_amount
            )
            adapted["associative_weight"] = max(
                0.1, params["associative_weight"] - 0.1 * adaptation_amount
            )
            adapted["temporal_weight"] = max(
                0.05, params["temporal_weight"] - 0.1 * adaptation_amount
            )
            ...
            adapted["confidence_threshold"] = max(
                0.05, params["confidence_threshold"] - 0.05 * adaptation_amount
            )
            adapted["similarity_weight"] = min(
                0.9, params["similarity_weight"] + 0.2 * adaptation_amount
            )
            adapted["associative_weight"] = max(
                0.05, params["associative_weight"] - 0.1 * adaptation_amount
            )
            adapted["max_candidates"] = int(
                params["max_candidates"] * (1 + 0.5 * adaptation_amount)
            )
            ...
            adapted["temporal_weight"] = min(
                0.4, params["temporal_weight"] + 0.3 * adaptation_amount
            )
    ...
    def _adapt_for_query_characteristics(
        ...
        if has_temporal_reference:
            ...
            adapted["similarity_weight"] = max(
                0.3, params["similarity_weight"] - reduction_per_weight
            )
            adapted["associative_weight"] = max(
                0.1, params["associative_weight"] - reduction_per_weight
            )
            adapted["activation_weight"] = max(
                0.05, params["activation_weight"] - reduction_per_weight
            )
                ...
                if isinstance(original_value, (int, float)) and isinstance(
                    ...
                    if isinstance(original_value, int):
                        result[key] = int(
                            round(
                                original_value
                                + self.adaptation_strength * (adapted_value - original_value)
                            )
                        )
                    else:
                        result[key] = original_value + self.adaptation_strength * (
                            adapted_value - original_value
                        )
                # For booleans and other values, use original unless adaptation_strength > 0.5
                else:
                    result[key] = (
                        adapted_value if self.adaptation_strength > 0.5 else original_value
                    )
            else:
