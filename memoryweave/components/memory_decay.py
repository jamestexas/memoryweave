...
class MemoryDecayComponent(Component):
    ...
    def _apply_memory_decay(self) -> None:
        ...
        try:
            ...
            # For legacy memory  
            elif hasattr(self.memory, "activation_levels") and isinstance(self.memory.activation_levels, np.ndarray):
                ...
                self.memory.activation_levels = self.memory.activation_levels * (1.0 - self.memory_decay_rate)
                
            # If ART clustering is enabled and category activations exist
            if (self.art_clustering_enabled and hasattr(self.memory, "category_activations") and 
                    isinstance(self.memory.category_activations, np.ndarray)):
                # Apply the same decay to category activations
                self.memory.category_activations = self.memory.category_activations * (1.0 - self.memory_decay_rate)
