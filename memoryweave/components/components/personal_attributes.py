...
class PersonalAttributeManager(Component):
    ...
    def process_query(self, query, context):
        ...
        if any(word in query.lower() for word in ["wife", "husband", "partner", "spouse"]):
            partner = self.personal_attributes["relationships"]["family"].get("wife") or \
                     self.personal_attributes["relationships"]["family"].get("husband")
            if partner:
    ...
    def _extract_demographics(self, text):
        ...
        # Extract occupation
        occupation_match = re.search(r"(?:work|job) as (?:a |an |)(.+?)(?:\.|\,|\s$|$)", text, re.IGNORECASE)
        if occupation_match:
            self.personal_attributes["demographics"]["occupation"] = occupation_match.group(1).strip()
