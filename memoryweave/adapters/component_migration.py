...
from memoryweave.interfaces.retrieval import IRetrievalStrategy, IPostProcessor
from memoryweave.interfaces.query import IQueryAnalyzer, IQueryAdapter
from memoryweave.interfaces.pipeline import (
    IComponent, ComponentType, ComponentID, IPipeline,
    IPipelineStage, IComponentRegistry, IPipelineBuilder
)
...
from memoryweave.adapters.memory_adapter import (
    LegacyMemoryAdapter, LegacyVectorStoreAdapter, LegacyActivationManagerAdapter
)
from memoryweave.adapters.retrieval_adapter import (
    LegacyRetrieverAdapter, NewToLegacyRetrieverAdapter
)
from memoryweave.adapters.pipeline_adapter import (
    LegacyToPipelineAdapter, PipelineToLegacyAdapter
)
...
class FeatureMigrator:
        ...
        if hasattr(legacy_memory, 'memory_retriever'):
            ...
            try:
                ...
                similarity_strategy = RetrievalFactory.create_retrieval_strategy(
                    ...
                    activation_adapter,
                    {
                        'similarity_threshold': getattr(retriever, 'default_confidence_threshold', 0.0),
                        'max_results': 10
                    }
                )
    ...
    def validate_migration(self, 
                          ...
                          test_queries: List[Union[str, np.ndarray]]) -> Dict[str, Any]:
        """
        ...
        for i, query in enumerate(test_queries):
            ...
            try:
                ...
                if hasattr(legacy_retriever, 'retrieve_for_context'):
                    ...
                    self._logger.warning(f"Unknown legacy retriever interface")
                    continue
        ...
        if validation_results['recall_metrics']:
            ...
            validation_results['avg_precision'] = sum(validation_results['precision_metrics']) / len(
    ...
    def _compare_retrieval_results(self, 
                                 legacy_results: List[Tuple], 
                                 migrated_results: List[Tuple]) -> bool:
        """
    ...
    def _calculate_result_metrics(self, 
                                legacy_results: List[Tuple], 
                                migrated_results: List[Tuple]) -> Tuple[float, float]:
        """
