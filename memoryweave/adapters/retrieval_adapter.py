...
class LegacyRetrieverAdapter(IRetrievalStrategy):
    ...
    def retrieve(
        self, query_embedding: np.ndarray, parameters: Optional[RetrievalParameters] = None
        ...
        if hasattr(self._legacy_retriever, "retrieve_memories"):
            ...
            legacy_results = self._legacy_retriever.retrieve_for_context(
                query_embedding=query_embedding, k=top_k, threshold=confidence_threshold
            )
        else:
...
class NewToLegacyRetrieverAdapter:
    ...
    def retrieve_for_context(
        ...
        parameters = {"max_results": k, "similarity_threshold": threshold}
