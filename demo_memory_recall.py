...
class MemoryRecallDemo:
    ...
    def _format_prompt(self, query, conversation_history=None, include_memories=False, memories=None):
    ...
    def generate_with_memory(self, query, conversation_history):
        ...
        prompt = self._format_prompt(query, conversation_history, include_memories=True, memories=memories)
    ...
    def generate_report(self):
        ...
        with_memory_accuracy = with_memory_correct / len(self.results_with_memory) if self.results_with_memory else 0
        without_memory_accuracy = without_memory_correct / len(self.results_without_memory) if self.results_without_memory else 0
...
personal_details_scenario = {
    ...
    'distraction': [
        {"query": "Let's talk about something different. What's the capital of France?", "expected": None},
